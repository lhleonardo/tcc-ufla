...
async function criaServidor() {
  const child = spawn("yarn", ["dev:server"], { cwd: "../implementacao-sem-ddd" });
 ...
 */
async function mataServidor(servidor) {
...
async function realizaRequisicao() {
  const token = uuid();
  const email = `${token.replaceAll("-", "").replaceAll(".", "")}@sample.com`;
...
function escreverTempo(tempoGasto) {
  ...
  fs.appendFileSync("./resultados/tempos2.txt", tempoGasto.toString()+"\n");
}
